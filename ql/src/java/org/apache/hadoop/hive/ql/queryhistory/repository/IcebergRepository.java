...
public class IcebergRepository extends AbstractRepository implements QueryHistoryRepository {
  ...
  protected void postInitTable(Table table) throws Exception {
    ...
    this.serializer = (Serializer) tableDesc.getDeserializer(conf);
  }
  ...
  private void prepareConfForWrite() {
    ...
    conf.set(HiveConf.ConfVars.HIVEQUERYID.varname, queryHistoryId);
    SessionState.getSessionConf().set(HiveConf.ConfVars.HIVEQUERYID.varname, queryHistoryId);
