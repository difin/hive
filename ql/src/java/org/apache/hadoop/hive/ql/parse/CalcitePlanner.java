...
public class CalcitePlanner extends SemanticAnalyzer {
      ...
      if (runCBO) {
        ...
        try {
          ...
          if (e instanceof CalciteSubquerySemanticException
              ...
              || e instanceof CalciteViewSemanticException) {
          ...
          } else if (isImpalaPlan) {
            // Non-CBO Impala plans are not supported, so propagate Impala exeception.
            throw e;
          }
  ...
  private CalcitePlan plan(boolean generateEnginePlan) throws SemanticException {
    ...
    Frameworks.PlannerAction<CalcitePlan> calcitePlannerAction = createPlannerAction(
            generateEnginePlan,
            prunedPartitions,
  ...
  protected Frameworks.PlannerAction<CalcitePlan> createPlannerAction(
          boolean generateEnginePlan,
          Map<String, PrunedPartitionList> partitionCache,
          Map<String, ColumnStatsList> colStatsCache,
          ColumnAccessInfo columnAccessInfo) {
    return new CalcitePlannerAction(generateEnginePlan, partitionCache, colStatsCache, columnAccessInfo, getQB());
  }
   ...
   */
  protected class CalcitePlannerAction implements Frameworks.PlannerAction<CalcitePlan> {
    ...
    protected RelNode applyMaterializedViewRewriting(RelOptPlanner planner, RelNode basePlan,
      ...
      try {
        ...
        materializations = materializations.stream()
            ...
            .map(materialization -> materialization.copyToNewCluster(
                    optCluster, functionHelper.getPartitionPruneRuleHelper()))
    ...
    private boolean isMaterializedViewRewritingByTextEnabled() {
      return conf.getBoolVar(ConfVars.HIVE_MATERIALIZED_VIEW_ENABLE_AUTO_REWRITING_SQL) &&
              ...
              !rootQB.isMaterializedView() && !ctx.isLoadingMaterializedView() && !getQB().isCTAS() &&
              rootQB.getIsQuery() &&
              rootQB.hasTableDefined() &&
              !forViewCreation &&
              conf.getEngine() == HiveConf.Engine.HIVE;
    }
    ...
    private RelNode genJoinRelNode(RelNode leftRel, String leftTableAlias, RelNode rightRel, String rightTableAlias, JoinType hiveJoinType,
      ...
      if (leftSemiJoin) {
        ...
        if (inputRels[0] != leftRel) {
          ...
          topRel = HiveRelFactories.HIVE_PROJECT_FACTORY
            .createProject(topRel, Collections.emptyList(), topFields, topFieldNames);
        }
    ...
    private RelNode genJoinLogicalPlan(QB qb, ASTNode joinParseTree, Map<String, RelNode> aliasToRel)
      ...
      if ((left.getToken().getType() == HiveParser.TOK_TABREF)
        ...
        leftRel = genJoinLogicalPlan(qb, left, aliasToRel);
    ...
    private Pair<RelNode, RowResolver> genConstraintFilterLogicalPlan(
      ...
      RexNode constraintUDF = RexNodeTypeCheck.genConstraintsExpr(
          conf, cluster.getRexBuilder(), getTargetTable(qb, dest), updating(dest), inputRR, functionHelper);
    ...
    private AggregateCall convertGBAgg(AggregateInfo agg, List<RexNode> gbChildProjLst,
      ...
      // type
      final SqlAggFunction aggregation = functionHelper.getCalciteAggregateFunction(
          agg.getAggregateName(), agg.isDistinct(),
    ...
    private AggregateInfo getHiveAggInfo(ASTNode aggAst, int aggFnLstArgIndx, RowResolver inputRR)
      ...
      if (aInfo == null) {
        ...
        RexNode exp = genRexNode((ASTNode) aggAst.getChild(0), inputRR, tcCtx, conf, queryHelper);
    ...
    private List<RexNode> getPartitionKeys(PartitionSpec ps,
      ...
      if (ps != null) {
        ...
        for (PartitionExpression pExpr : pExprs) {
          ...
          tcCtx.setAllowStatefulFunctions(true);
          tcCtx.setUnparseTranslator(unparseTranslator);
          RexNode exp = genRexNode(pExpr.getExpression(), inputRR, tcCtx, conf, queryHelper);
          pKeys.add(exp);
        }
      }
    ...
    private List<RexFieldCollation> getOrderKeys(OrderSpec os,
      ...
      if (os != null) {
        ...
        for (OrderExpression oExpr : oExprs) {
          ...
          tcCtx.setAllowStatefulFunctions(true);
          tcCtx.setUnparseTranslator(unparseTranslator);
          RexNode ordExp = genRexNode(oExpr.getExpression(), inputRR, tcCtx, conf, queryHelper);
    ...
    private Pair<RexNode, TypeInfo> genWindowingProj(WindowExpressionSpec wExpSpec, RelNode srcRel)
      ...
      if (wExpSpec instanceof WindowFunctionSpec) {
        ...
        final SqlAggFunction calciteAggFn = functionHelper.getCalciteAggregateFunction(
      ...
      for (int i = startPosn; i < exprList.getChildCount(); ++i) {
        ...
        if(isSubQuery) {
          ...
          if (expr.getType() == HiveParser.TOK_ALLCOLREF) {
            ...
            RexNode expression = genRexNode(expr, inputRR, tcCtx, conf, queryHelper);
    ...
    private RelNode genLogicalPlan(QB qb, boolean outerMostQB,
      ...
      if (qb.getParseInfo().getJoinExpr() != null) {
        srcRel = genJoinLogicalPlan(qb, qb.getParseInfo().getJoinExpr(), aliasToRel);
      } else {
  ...
  protected Table getTableObjectByName(String tabName, boolean throwException) throws HiveException {
    ...
    if (!tabNameToTabObject.containsKey(fullyQualName)) {
      Table table = db.getTable(dbName, tableName, metaTable, throwException, true);
      if (table != null) {
  ...
  RexNode genRexNode(ASTNode expr, RowResolver input,
    ...
    tcCtx.setUnparseTranslator(unparseTranslator);
    return genRexNode(expr, input, tcCtx, conf, queryHelper);
  }
  ...
  Map<ASTNode, RexNode> genAllRexNode(ASTNode expr, RowResolver input, RexBuilder rexBuilder)
    ...
    tcCtx.setUnparseTranslator(unparseTranslator);
    return genAllRexNode(expr, input, tcCtx, conf, this.queryHelper);
  }
  ...
  public static RexNode genRexNode(ASTNode expr, RowResolver input,
      TypeCheckCtx tcCtx, HiveConf conf, EngineQueryHelper queryHelper) throws SemanticException {
    RexNode cached = null;
    if (tcCtx.isUseCaching()) {
      cached = getRexNodeCached(expr, input, tcCtx, queryHelper);
    }
    if (cached == null) {
      Map<ASTNode, RexNode> allExprs = genAllRexNode(expr, input, tcCtx, conf, queryHelper);
  ...
  private static RexNode getRexNodeCached(ASTNode node, RowResolver input,
      TypeCheckCtx tcCtx, EngineQueryHelper queryHelper) throws SemanticException {
   ...
   */
  protected static class CalcitePlan {
