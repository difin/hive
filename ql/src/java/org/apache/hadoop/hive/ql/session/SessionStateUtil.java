...
public class SessionStateUtil {
  ...
  public static void setConflictDetectionFilter(Configuration conf, String tableName, ExprNodeDesc filterExpr) {
    ...
    Optional<ExprNodeGenericFuncDesc> prevFilterExpr = getConflictDetectionFilter(conf, tableName);
    if (prevFilterExpr.isPresent()) {
        if (!prevFilterExpr.get().isSame(filterExpr)) {
          ExprNodeDesc disjunction = null;
          try {
            disjunction = ExprNodeDescUtils.disjunction(prevFilterExpr.get(), filterExpr);
