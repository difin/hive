...
public class EnforceReadOnlyDatabaseHook implements ExecuteWithHookContext {
  ...
  private static void checkReadOnlyDbAsOutput(HookContext hookContext) throws HiveException {
    ...
    for (WriteEntity output : outputs) {
      ...
      if (output.getDatabase() != null) {
        dbName = output.getDatabase().getName();
      } else if (output.getTable() != null) {
        dbName = output.getTable().getDbName();
      } else if (output.getPartition() != null) {
        dbName = output.getPartition().getTable().getDbName();
      } else {
        // Ignore other types.
        continue;
      }
      ...
      final Database database = hive.getDatabase(dbName);
      ...
      if (Boolean.valueOf(parameters.get(READONLY))) {
      ...
      if (rootTasks.size() == 1) {
        ...
        if (rootWork instanceof DDLWork) {
          ...
          if (ddlDesc instanceof AlterDatabaseSetPropertiesDesc) {
            ...
            return !Boolean.valueOf(properties.get(READONLY));
          }
        }
      }
    }
  ...
  private static boolean isReadOnlyOperation(HiveOperation hiveOperation) {
    ...
    return Sets.newHashSet("SHOW", "DESC").stream().anyMatch(hiveOperation.name()::startsWith);
  }
}
