...
public class TestDriver {
  ...
  public void testDriverContextQueryErrorMessageCompileTime() {
    ...
    try {
      ...
      String message = ((CommandProcessorException)e).getErrorMessage();
      ...
      Assert.assertTrue("Exception message is not as expected, got: " + message,
          message.startsWith("FAILED: ParseException line 1:0 cannot recognize input near"));
    } finally {
      driver.close();
    }
  }
  ...
  public void testDriverContextQueryErrorMessageRuntime() {
    ...
    try {
      ...
      // sanity check: the message is as expected
      Assert.assertTrue("Exception message is not as expected, got: " + message,
          message.startsWith("FAILED: Execution Error, return code 1 from org.apache.hadoop.hive.ql.exec.tez.TezTask"));
    } finally {
      driver.close();
    }
  }
