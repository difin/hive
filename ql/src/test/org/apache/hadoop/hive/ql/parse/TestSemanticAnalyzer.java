...
public class TestSemanticAnalyzer {
  ...
  public void testQueryTypes() throws Exception {
     ...
     *
     * Fun fact Cloudera downstream: we already rewritten this to ANALYZE_TABLE, e.g. in the huge CDPD-26117/HIVE-23274
     * patch there is only type: ANALYZE_TABLE for all the ANALYZE TABLE statements.
     */
    checkQueryType("ANALYZE TABLE table1 COMPUTE STATISTICS", QueryType.STATS, "ANALYZE_TABLE");
  ...
  private void checkQueryType(String query, QueryProperties.QueryType expectedQueryType, String expectedOperation)
    ...
    ASTNode astNode = ParseUtils.parse(query, ctx);
    QueryState queryState = new QueryState.Builder().withHiveConf(conf).build();
    HiveConf.setVar(conf, HiveConf.ConfVars.HIVEQUERYID, "test_query_id");
