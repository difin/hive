...
public class TestCLIServiceConnectionLimits {
  ...
  private void testConnectionLimits() {
    ...
    sessionConf.put("hive.conf.internal.variable.list", "test");
    try {
      ...
      for (int i = 0; i < 3; i++) {
        try {
          ...
          Assert.assertEquals(
              "Failed to open new session: Cannot modify hive.conf.internal.variable.list at runtime. It is in the list of parameters that can't be modified at runtime or is prefixed by a restricted variable",
              he.getMessage());
        }
      }
    } finally {
