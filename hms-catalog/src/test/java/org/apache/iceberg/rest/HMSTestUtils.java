...
public class HMSTestUtils {
  private static final Map<Integer, Thread> map = new HashMap<>();

  private static final Logger LOG = LoggerFactory.getLogger(HMSTestUtils.class);
  ...
  public static void startMetaStore(final int port, final HadoopThriftAuthBridge bridge, Configuration conf,
    ...
    HMSTestUtils.loopUntilHMSReady(msHost, port, startedBackgroundThreads);
    ...
    if (serviceDiscMode != null && serviceDiscMode.equalsIgnoreCase("zookeeper")) {
      HMSTestUtils.loopUntilZKReady(conf, msHost, port);
    }
  }
  ...
  public static int startMetaStoreWithRetry(final HadoopThriftAuthBridge bridge) throws Exception {
    return HMSTestUtils.startMetaStoreWithRetry(bridge, MetastoreConf.newMetastoreConf());
  }
  ...
  public static int startMetaStoreWithRetry(Configuration conf, boolean keepWarehousePath) throws Exception {
    return HMSTestUtils.startMetaStoreWithRetry(HadoopThriftAuthBridge.getBridge(), conf, false, keepWarehousePath, false, false);
  }
  ...
  public static int startMetaStoreWithRetry() throws Exception {
    return HMSTestUtils.startMetaStoreWithRetry(HadoopThriftAuthBridge.getBridge(),
        MetastoreConf.newMetastoreConf());
  }

  public static int startMetaStoreWithRetry(HadoopThriftAuthBridge bridge,
                                            Configuration conf) throws Exception {
    return HMSTestUtils.startMetaStoreWithRetry(bridge, conf, false, false, false, false);
  }

  public static int startMetaStoreWithRetry(HadoopThriftAuthBridge bridge,
                                            Configuration conf, boolean withHouseKeepingThreads)
          throws Exception {
    return HMSTestUtils.startMetaStoreWithRetry(bridge, conf, false, false, withHouseKeepingThreads, false);
  }
  ...
  public static int startMetaStoreWithRetry(HadoopThriftAuthBridge bridge, Configuration conf, boolean keepJdbcUri,
    ...
    for (int tryCount = 0; tryCount < HMSTestUtils.RETRY_COUNT; tryCount++) {
      try {
        ...
        HMSTestUtils.startMetaStore(metaStorePort, bridge, conf, withHouseKeepingThreads, waitForHouseKeepers);
  ...
  private static void loopUntilHMSReady(String msHost, int port, AtomicBoolean houseKeepingStarted) throws Exception {
    ...
    LOG.info(HMSTestUtils.getAllThreadStacksAsString());
    throw exc;
  }
  ...
  private static String getAllThreadStacksAsString() {
    ...
    for (Map.Entry<Thread, StackTraceElement[]> entry : threadStacks.entrySet()) {
      ...
      HMSTestUtils.addStackString(entry.getValue(), sb);
    }
    return sb.toString();
  }
