...
public class HMSCatalogActor implements HiveActor {
  ...
  public List<Table> listTables(String database, List<String> tableNames) throws TException {
    return tableNames.isEmpty()
        ...
        : run(h -> h.get_table_objects_by_name(database, tableNames));
  }
  ...
  @Override
  public boolean isTableAccessible(String databaseName, String tableName) {
    IHMSHandler handler = null;
    try {
      try {
        handler = handlers.borrowObject();
      } catch (Exception e) {
        LOG.error("unable to borrow handler", e);
        return false;
      }
      return HMSServletSecurity.isTableAccessible(handler, databaseName, tableName);
    } catch (Exception e) {
      // any error, especially security means no-access
      LOG.info("{}.{} not accessible", databaseName, tableName, e);
    } finally {
      if (handler != null) {
        try {
          handlers.returnObject(handler);
        } catch (Exception e) {
          LOG.error("unable to return handler", e);
        }
      }
    }
    return false;
