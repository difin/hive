 ...
 */
public class HiveCachingCatalog<CATALOG extends Catalog & SupportsNamespaces> implements Catalog, SupportsNamespaces, TableAccess {
  ...
  protected final Map<TableIdentifier, Map<String, Integer>> userRights;
  private final CATALOG catalog;
  private final boolean caseSensitive;

  private final int READ_MASK = 4;
  private final int WRITE_MASK = 2;
  private final int META_MASK = 1;


  @SuppressWarnings("checkstyle:VisibilityModifier")
  protected HiveCachingCatalog(CATALOG catalog, long expirationIntervalMillis, boolean cacheAuthorization) {
    ...
    this.userRights = cacheAuthorization? new ConcurrentHashMap<>() : null;
  ...
  @Override
  public boolean isReadAllowed(TableIdentifier ident, String user) {
    TableIdentifier canonical = canonicalizeIdentifier(ident);
    if (userRights == null) {
      return (computeUserRights(ident, user) & READ_MASK) != 0;
    }
    final Map<String, Integer> allowed = userRights.computeIfAbsent(canonical, k->new HashMap<>());
    synchronized(allowed) {
      return (allowed.computeIfAbsent(user, u -> computeUserRights(ident, u)) & READ_MASK) != 0;
    }
  }

  private int computeUserRights(TableIdentifier ident, String user) {
    ...
    if (actor != null) {
      ...
      try {
        ...
        accessible = actor.isTableAccessible(dbName, tableName);
        if (accessible) {
          return READ_MASK;
        } else {
          LOG.info("Table {} is not accessible to {}", ident, user);
        }
  ...
  HiveCachingCatalog<C> wrap(C catalog, long expirationIntervalMillis, boolean cacheAuthorization) {
    return new HiveCachingCatalog(catalog, expirationIntervalMillis, cacheAuthorization);
  }
  ...
  public void invalidateTable(TableIdentifier ident) {
    ...
    TableIdentifier canonicalized = canonicalizeIdentifier(ident);
    if (userRights != null) {
      userRights.remove(canonicalized);
    }
  ...
  class MetadataTableInvalidatingRemovalListener
    ...
    public void onRemoval(TableIdentifier tableIdentifier, Table table, RemovalCause cause) {
      ...
      if (RemovalCause.EXPIRED.equals(cause)) {
        ...
        if (userRights != null) {
          TableIdentifier canonical = canonicalizeIdentifier(tableIdentifier);
          userRights.remove(canonical);
        }
      }
    }
  }
