...
public class TestMRCompactorOnTez extends CompactorOnTezTest {
  ...
  public void testCompactorGatherStats() throws Exception{
    ...
    executeStatementOnDriverSilently("drop table if exists " + dbName + "." + tableName, driver);
    executeStatementOnDriverSilently("create table " + dbName + "." + tableName +
        " (a INT) STORED AS ORC TBLPROPERTIES ('transactional'='true')", driver);
    executeStatementOnDriverSilently("insert into " + dbName + "." + tableName + " values(1)", driver);

    // Make sure we do not have statistics for this table yet
    // Compaction generates stats only if there is any
    IMetaStoreClient msClient = new HiveMetaStoreClient(conf);
    executeStatementOnDriverSilently("analyze table " + dbName + "." + tableName + " compute statistics for columns", driver);
    executeStatementOnDriverSilently("insert into " + dbName + "." + tableName + " values(2)", driver);
    ...
    executeStatementOnDriverSilently("insert into " + dbName + "." + tableName + " values(3)", driver);
    executeStatementOnDriverSilently("alter table " + dbName + "." + tableName + " set tblproperties('compactor.mapred.job.queue.name'='" +
    ...
    executeStatementOnDriverSilently("insert into " + dbName + "." + tableName + " values(4)", driver);
    conf.setVar(HiveConf.ConfVars.PREEXECHOOKS, HiveProtoLoggingHook.class.getName());
    CompactorTestUtil.runCompaction(conf, dbName, tableName, CompactionType.MAJOR, false,
