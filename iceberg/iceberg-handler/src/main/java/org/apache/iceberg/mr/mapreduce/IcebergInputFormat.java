...
public class IcebergInputFormat<T> extends InputFormat<Void, T> {
  ...
  private static final class IcebergRecordReader<T> extends RecordReader<Void, T> {
    ...
    private CloseableIterable<T> openVectorized(
    ...
    private CloseableIterable<T> open(
        FileScanTask currentTask, Schema readSchema,
        Function<DeleteFilter<T>, Map<String, PositionDeleteIndex>> positionIndex) {
      switch (inMemoryDataModel) {
        ...
        case HIVE:
          return openVectorized(currentTask, readSchema, positionIndex);
        case GENERIC:
          DeleteFilter deletes = new GenericDeleteFilter(table.io(), currentTask, table.schema(), readSchema);
          Schema requiredSchema = deletes.requiredSchema();
          return deletes.filter(openGeneric(currentTask, requiredSchema), positionIndex.apply(deletes));
        default:
