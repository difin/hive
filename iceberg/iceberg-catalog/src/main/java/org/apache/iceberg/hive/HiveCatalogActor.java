...
public class HiveCatalogActor implements HiveActor {
  ...
  /** The table authorizer (external authorization integration). */
  private TableAccessAuthorizer<IMetaStoreClient> tableAuth = (c, d, t) -> true;

  public void setTableAccessAuthorizer(TableAccessAuthorizer<IMetaStoreClient> taa) {
    tableAuth = taa;
  }

  @FunctionalInterface
  public interface TableAccessAuthorizer<C> {
    boolean isTableAccessible(C actor,  String databaseName, String tableName);
  }
  ...
  private boolean isTableAccessible(IMetaStoreClient client, String databaseName, String tableName) {
    return tableAuth == null || tableAuth.isTableAccessible(client, databaseName, tableName);
  }

  @Override
  public boolean isTableAccessible(String databaseName, String tableName) throws TException, InterruptedException {
    return clients.run(client -> isTableAccessible(client, databaseName, tableName));
  }
