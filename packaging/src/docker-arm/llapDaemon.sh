...
export LLAP_LOG4J2_PROPERTIES_FILE_NAME="${LLAP_LOG4J2_PROPERTIES_FILE_NAME:-llap-daemon-edws-log4j2.properties}"
...
case $startStop in
  ...
  (start)
    ...
    # For running in k8s we don't need to capture pid (required only for explicity stop)
    #echo $! > $pid
    # capture the ulimit output
    echo "ulimit -a" >> $logOut
    ulimit -a >> $logOut 2>&1

    exec "$LLAP_DAEMON_BIN_HOME"/runLlapDaemon.sh run
    ;;
