...
export JVM_PID="$$"
CLASSPATH="${LLAP_DAEMON_CONF_DIR}:/custom-jars/*:/custom-jars/lib/*:${CLASSPATH}:${UDF_JARS_PATH}/conf:${UDF_JARS_PATH}/lib/udfs/*"
exec "$JAVA" -Dproc_llapdaemon ${LLAP_DAEMON_OPTS} ${SERVICE_OPTS} -classpath "$CLASSPATH" $CLASS "$@"
