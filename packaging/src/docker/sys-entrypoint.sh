...
# Information schema
$HIVE_HOME/bin/schematool -dbType hive -metaDbType postgres -initSchema
if [ $? -eq 0 ]; then
...
if [ "${CREATE_LOGS_TABLE}" == "true" ]; then
  ...
  $HIVE_HOME/bin/schematool -dbType hive -metaDbType postgres -createLogsTable ${REMOTE_LOGS_PATH} -retentionPeriod ${LOG_RETENTION_PERIOD}
  if [ $? -eq 0 ]; then
