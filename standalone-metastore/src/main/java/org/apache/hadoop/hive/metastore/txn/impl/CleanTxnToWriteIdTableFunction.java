...
public class CleanTxnToWriteIdTableFunction implements TransactionalFunction<Void> {
  ...
  private static String noMinHistoryLevelSql = "SELECT MIN(\"RES\".\"ID\") AS \"ID\" FROM (" +
      ...
      "   UNION" +
      " SELECT MIN(\"TXN_ID\") AS \"ID\" FROM \"TXNS\" " +
      "   WHERE \"TXN_STATE\" = :abortedState OR \"TXN_STATE\" = :openState) \"RES\"";

  private final boolean useMinHistoryLevel;
  private final long minTxnIdSeenOpen;

  public CleanTxnToWriteIdTableFunction(boolean useMinHistoryLevel, long minTxnIdSeenOpen) {
