...
public class FindPotentialCompactionsFunction implements TransactionalFunction<Set<CompactionInfo>> {
  ...
  public FindPotentialCompactionsFunction(SQLGenerator sqlGenerator, Configuration conf, int abortedThreshold,
                                          long abortedTimeThreshold, long lastChecked) {
  ...
  public Set<CompactionInfo> execute(MultiDataSourceJdbcResource jdbcResource) throws MetaException {
    Set<CompactionInfo> candidates = new HashSet<>(jdbcResource.execute(
        new CompactionCandidateHandler(sqlGenerator, lastChecked, fetchSize)));
    int remaining = fetchSize - candidates.size();
    if (collectAbortedTxns && remaining > 0) {
      candidates.addAll(jdbcResource.execute(new AbortedTxnHandler(sqlGenerator, abortedTimeThreshold, abortedThreshold, remaining)));
    }
    return candidates;
  }
}
