...
public class DirectSqlAggrStats {
  ...
  public DirectSqlAggrStats(PersistenceManager pm, Configuration conf, String schema) {
    ...
    if (configBatchSize == DETECT_BATCHING) {
      configBatchSize = DatabaseProduct.needsInBatching(dbType) ? 1000 : NO_BATCHING;
    }
  ...
  public ColumnStatistics getTableStats(final String catName, final String dbName,
    ...
    Batchable<String, Object[]> b = new Batchable<String, Object[]>() {
      ...
      public List<Object[]> run(List<String> input) throws MetaException {
        String queryText = queryText0 + MetastoreDirectSqlUtils.makeParams(input.size()) + ")";
      ...
      public List<Object[]> run(final List<String> inputColNames)
        ...
        Batchable<String, Object[]> partitionBatchesFetcher = new Batchable<String, Object[]>() {
          ...
          public List<Object[]> run(List<String> inputPartNames)
              throws MetaException {
            String queryText =
                String.format(queryText0, MetastoreDirectSqlUtils.makeParams(inputColNames.size()), MetastoreDirectSqlUtils.makeParams(inputPartNames.size()));
            long start = doTrace ? System.nanoTime() : 0;
            Query<?> query = pm.newQuery("javax.jdo.query.SQL", queryText);
            try {
              Object qResult = MetastoreDirectSqlUtils.executeWithArray(query,
                  MetastoreDirectSqlUtils.prepareParams(catName, dbName, tableName, inputPartNames, inputColNames, engine), queryText);
  ...
  public List<MetaStoreUtils.ColStatsObjWithSourceInfo> getColStatsForAllTablePartitions(String catName, String dbName,
                                                                                         boolean enableBitVector, boolean enableKll) throws MetaException {
    ...
    List<MetaStoreUtils.ColStatsObjWithSourceInfo> colStatsForDB = new ArrayList<MetaStoreUtils.ColStatsObjWithSourceInfo>();
    try {
