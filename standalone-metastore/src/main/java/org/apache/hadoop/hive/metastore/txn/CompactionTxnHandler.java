...
import org.apache.hadoop.hive.metastore.txn.jdbc.commands.DeleteFailedTxnsCommand;
...
class CompactionTxnHandler extends TxnHandler {
  ...
  public Set<CompactionInfo> findPotentialCompactions(int abortedThreshold,
    ...
    return new FindPotentialCompactionsFunction(sqlGenerator, conf, abortedThreshold, abortedTimeThreshold, lastChecked).execute(jdbcResource);
  }
  ...
  public List<CompactionInfo> findReadyToClean(long minOpenTxnWaterMark, long retentionTime) throws MetaException {
    return jdbcResource.execute(new ReadyToCleanHandler(sqlGenerator, conf, minOpenTxnWaterMark, retentionTime));
  }

  @Override
  @RetrySemantics.ReadOnly
  public List<CompactionInfo> findReadyToCleanAborts(long abortedTimeThreshold, int abortedThreshold) throws MetaException {
    return jdbcResource.execute(new ReadyToCleanAbortHandler(sqlGenerator, conf, abortedTimeThreshold, abortedThreshold));
  }
  ...
  public void cleanEmptyAbortedTxns() throws MetaException {
    jdbcResource.execute(new DeleteFailedTxnsCommand());
    LOG.debug("Removed empty Aborted and Committed transactions from TXNS");
  }
  ...
  public long findMinOpenTxnIdForCleaner() throws MetaException {
    ...
    return new MinOpenTxnIdWaterMarkFunction().execute(jdbcResource);
  }
  ...
  public long findMinTxnIdSeenOpen() {
    ...
    try {
      ...
      if (e.getCause() instanceof SQLException) {
        if (DatabaseProduct.isTableNotExistsError(dbProduct, (SQLException) e.getCause())) {
