...
public final class TestTxnDbUtil {
  ...
  private static String getSchemaRootPath() {
    ...
    if (StringUtils.isNotEmpty(hiveRoot)) {
      return ensurePathEndsInSlash(hiveRoot) + "standalone-metastore/target/tmp/";
    } else {
  ...
  public static void cleanDb(Configuration conf) throws Exception {
    ...
    while(++retryCount <= 3) {
      ...
      try {
        ...
        success &= truncateTable(conn, stmt, "MIN_HISTORY_WRITE_ID");
        ...
        success &= truncateTable(conn, stmt, "MIN_HISTORY_LEVEL");
        ...
        success &= truncateTable(conn, stmt, "COMPACTION_METRICS_CACHE");
