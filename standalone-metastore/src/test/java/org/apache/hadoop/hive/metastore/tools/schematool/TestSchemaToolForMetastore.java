...
//    dbs.add(new Object[] { new Mariadb() }); Disabled due to HIVE-27749
  ...
  public void testValidateSchemaTables() throws Exception {
    ...
    String[] scripts = new String[] {
            "update CDH_VERSION set SCHEMA_VERSION='" + incrementedDBVersion + "' where VER_ID=1;"
    };
    scriptFile = generateTestScript(scripts);
    schemaTool.execSql(scriptFile.getPath());
    ...
    scripts = new String[] {
            "update CDH_VERSION set SCHEMA_VERSION='" + DBVersion + "' where VER_ID=1;"
    };
    scriptFile = generateTestScript(scripts);
    schemaTool.execSql(scriptFile.getPath());
    isValid = validator.validateSchemaTables(conn);
